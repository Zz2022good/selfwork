...
void matmul(cudaStream_t stream, int M, int N, int K, float alpha, float *A,  float *B, float beta, float *C) {
  ...
  static_assert((K10_NUM_THREADS * 4) % K10_BK == 0, "error"
                );
  static_assert((K10_NUM_THREADS * 4) % K10_BN == 0, "error"
                );
  static_assert(K10_BN % (16 * K10_TN) == 0,"error"
                );
  static_assert(K10_BM % (16 * K10_TM) == 0,"error"
                );
  static_assert((K10_BM * K10_BK) % (4 * K10_NUM_THREADS) == 0,"error"
                );
  static_assert((K10_BN * K10_BK) % (4 * K10_NUM_THREADS) == 0,"error"
               );
